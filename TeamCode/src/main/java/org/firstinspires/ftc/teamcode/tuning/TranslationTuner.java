...
@Configurable
public class TranslationTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Vector2d target = new Vector2d(72, targetY);

        while (opModeIsActive()) {
            if (gamepad1.aWasPressed()) {
                follower.queue(new PathBuilder().setShouldPrecalculate(false).lineThrough(0, target).build());
            }
            follower.updateCoefficients(null, new FollowerPIDCoefficients(PIDCoefficients.empty(), new PIDCoefficients(p, i, d),PIDCoefficients.empty()));
            follower.update();
